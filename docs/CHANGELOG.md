...
- 2026-06-05: **Agent now installs+starts OpenSSH Server on Windows** (capability, sshd auto-start, firewall port 22, ACL fix) so SSH actually works after install. Confirmed VM<->client NetBird reachability (ping ok).
- 2026-06-05: **FULL E2E VERIFIED** — client added via UI -> agent installed -> NetBird joined -> OpenSSH Server auto-installed -> port 22 reachable from another mesh peer (banner: OpenSSH_for_Windows_9.5). Complete remote-access loop proven end to end.
